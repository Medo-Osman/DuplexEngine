...
protected:
	...
	bool m_destroyActionOnComplete = false;

public:
	void initializeSegment(Entity* entity, bool destroyActionOnComplete = false)
	{
		m_bossEntity = entity;
		m_destroyActionOnComplete = destroyActionOnComplete;
	}

	Vector3 m_entityOffset;
	...
	virtual void update(const float& dt)
	{
		...
		else if (m_actionQueue.size() > 0 || !m_destroyActionOnComplete)
			nextAction();
	}
